...
void reversePic(Picture *pic) {
	//pentru a face reverse unei imagini, trebuie facut swap intre randurile acesteia

	for(int i = 0; i < pic->height/2; i++) {
		for(int j = 0; j < pic->width; j++) {
			Pixel aux = *pic->pix_array[i];
			*pic->pix_array[i] = *pic->pix_array[pic->height - (j + 1)];
			*pic->pix_array[pic->height - (j + 1)] = aux;
			}
		}
	}
